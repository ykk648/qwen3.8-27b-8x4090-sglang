...
为验证“少跨 PCIe 通信的 TP4 是否更适合 Codex”，在相同 128K、DFlash2、并发 1、256-token
输出条件下测试 `TP4 + EP1`：

| 实际 prompt tokens | 4 卡 TP4+EP1 | 8 卡 TP8+EP2 | 结论 |
|---:|---:|---:|---|
| 约 60,958 | 56.97 tok/s | 57.54 tok/s | 基本持平 |
| 约 111,528 | 30.27 tok/s | **34.22 tok/s** | 8 卡快约 13% |

4 卡完整支持 128K 和 DFlash2，适合作为节省 GPU 的备选；但用户的首要目标是 Codex 长会话
流畅度，最终保留 8 卡 TP8+EP2。在 61K 以内两者差异很小，在 100K+ 活跃历史时选择 8 卡。
